<template>
    <section>
        <div class="botones">
            ...
            &nbsp;
            <!--Boton para imprimir-->
            <button @click.prevent="registroPDF" class="btn btn-sm btn-outline-primary" v-if="registros.length > 0">
                Imprimir <i class="fa fa-print"></i>
            </button>
            &nbsp;
            <!--Agregamos un boton nuevo para Excel-->
            <button class="btn btn-sm btn-outline-primary" v-if="registros.length > 0">
                <download-excel :data="registros" type="xlsx" name="registro.xlsx">
                    Excel <i class="fa fa-file-excel-o"></i>
                </download-excel>
            </button>
        </div>
    </section>
    <table class="table table-striped table-bordered" id="tablaRegistro">
        <caption><h3>Registro</h3></caption>
        ...
        <tbody>
            ...
            <tr v-if="registros.length == 0">
                <td class="centrado" colspan="6">Sin registro registrado</td>
            </tr>
                ...
                <td class="centrado">
                    <div class="btn-group" role="group" aria-label="Basic outlined example">
                        <button type="button" class="btn btn-sm btn-outline-primary">
                            <!--Boton modificar-->
                            <RouterLink class="nav-link item" :to = "{path: '/registro/'+ registro.id_registro + '/editar'}"><i class="fa fa-pencil"></i></RouterLink>
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-danger">
                            <!--Boton eliminar-->
                            <RouterLink class="nav-link item" :to = "{path: '/registro/'+ registro.id_registro + '/borrar'}"><i class="fa fa-trash"></i></RouterLink>
                        </button>
                    </div>
                </td>
            </tr>
...
<script setup lang="ts">
    ...
    const { traeRegistro, registros} = useRegistro();
    //Cuando la pagina es visible y esta cargada
    onMounted(async () => {
        await traeRegistro();
    });

    const registroPDF = async () => {
    const pagina = document.getElementById('tablaRegistro');
    if (!pagina) {
        console.error("Elemento 'tablaRegistro' no encontrado.");
        return;
    }
